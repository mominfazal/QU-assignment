...
def get_data_from_twitter(query,tweetID):
	...
	try:
		...
		if(tweetID is None): # This will always be None
		...
		#At this moment userData is actually donaldTrumpData
...
def get_data_from_cnn(query,start):
	...
	if query is None:
			query = 'donald trump'
	if start == 1:
		url = 'https://search.api.cnn.io/content?q='+query+'&size=25'  # Before scraping! I decided to analyse the request flow of the CNN website and there I found this API which fetches all the news data for the page.
	else:
		fromNo = ((int(start)-1)*25)+1
		url = 'https://search.api.cnn.io/content?q='+query+'&size=25&page='+str(start)+'&from='+str(fromNo) # Little more examining and found the pagination logic.
	print url
	try:
		query_data = requests.get(url)
		query_data = json.loads(query_data.content)
